...
const BIP39_ENGLISH: &[&str] = &[
    ...
    "girl", "give", "glad", "glance", "flare", "glass", "glide", "glimpse", "globe", "gloom",
    ...
    "modify", "mom", "moment", "monitor", "monkey", "monster", "month", "moon", "moral",
    ...
    "nasty", "nation", "nature", "near", "neck", "need", "negative", "neglect", "neither",
    ...
    "pilot", "pink", "pioneer", "pipe", "pirate", "pistol", "pitch", "pizza", "place", "planet",
    "plastic", "plate", "play", "player", "pleasure", "plenty", "plot", "pluck", "plug",
    ...
    "right", "rigid", "ring", "riot", "rip", "ripe", "risk", "ritual", "rival", "river",
    ...
    "suffer", "sugar", "suggest", "suit", "sun", "sunny", "sunset", "super", "supply", "support",
    "suppose", "sure", "surprise", "surround", "survey", "suspect", "sustain", "swallow",
...
impl RecoverySeed {
    ...
    pub fn from_phrase(phrase: &str) -> Result<Self, String> {
        ...
        for word in &words {
            if !BIP39_ENGLISH.contains(&word.as_str()) {
                return Err(format!("invalid word: {}", word));
            }
        }

        Ok(RecoverySeed { words })
    }

    pub fn to_bytes(&self) -> [u8; 32] {
        let entropy = words_to_entropy(&self.words);
...
pub fn phrase_to_string(seed: &RecoverySeed) -> String {
    seed.words.join(" ")
}
...
fn generate_entropy() -> [u8; 16] {
    ...
    let last_byte = entropy[15] & 0xF0;
    entropy[15] = last_byte | checksum_byte;
...
fn words_to_entropy(words: &[String]) -> [u8; 16] {
    let mut bits = Vec::new();
    for word in words {
        if let Some(idx) = BIP39_ENGLISH.iter().position(|&w| w == word) {
            for i in (0..11).rev() {
...
pub fn restore_keypair_from_seed(seed_bytes: &[u8; 32]) -> crate::identity::RootKeypair {
    crate::identity::restore_keypair(seed_bytes)
}
...
mod tests {
    ...
    fn test_seed_roundtrip() {
        ...
        let phrase = phrase_to_string(&seed);
        let restored = RecoverySeed::from_phrase(&phrase).unwrap();
        assert_eq!(seed.words, restored.words);
    }

    #[test]
    fn test_entropy_recovery() {
        let seed = RecoverySeed::generate();
    ...
    fn test_invalid_phrase() {
        let result = RecoverySeed::from_phrase("not enough words");
